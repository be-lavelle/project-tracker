...
export const Column = ({ columnData, handleAdd, refreshList, columnInfo }: ColumnProps) => {
    ...
    let projects = columnData.sort((a: { order: number; }, b: { order: number; }) => {
        ...
        return <Project key={project.id} name={project.name} columnInfo={columnInfo} description={project.description} id={project.id} order={project.order} length={columnData.length} projectLabels={project.labels} refreshList={refreshList} />
    })
